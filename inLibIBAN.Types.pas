...
type
  ...
  public
    IBAN: string; // size(4)
                  // Codigo del Pais (ISO 3166-1) + Digito verificador
    CCC: string;  // size(Varia Segun Pais), maxsize(30)
                  // En el caso de España (Entidad + Oficina + DC + Cuenta)

    // HELPERs
    function ToFull(Sep:string=''): string;
                                    // IBAN Completo (ES1720852066623456789011)
                                    //IBAN-ESP: maxsize(24) | IBAN:maxsize(34)
  ...
  { Info Bancaria del Codigo del Pais,
...
begin
  ...
  //Si tiene el prefijo lo quitamos.
  if SameText(Prefix, _PrefixFormatPapelIBAN) then
  begin
...
constructor TrBancoCuentaInfo.BuildESP(inIBAN,
                                       inEntidad,
                                       inOficina,
                                       inDC,
                                       inCuenta: string);
var
  CCCFull: string;
begin
  CCCFull := TrBancoCCCInfoESP.Build(inEntidad,
                                     inOficina,
                                     inDC,
                                     inCuenta).ToCCC;
...
begin
  ...
  //   1) Tomas el numero de cuenta sin espacios ni guiones ni nada
...
function TrBancoIBANInfo.IsValid_DC(inCCC: String;
...
function TrBancoIBANInfo.IsValid(inCCC: String;
                                 Errores: TStringList=nil): Boolean;
begin
...
constructor TrBancoCCCInfoESP.Build(inEntidad,
                                    inOficina,
                                    inDC,
                                    inCuenta: string);
begin
